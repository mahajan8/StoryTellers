...
export default class Otp extends Component {
    ...
    submitOTP() {
        if(this.isComplete(this.state.otp)) {
            ...
            this['otp'+this.state.focus].blur()
            this.setState({
                otp: ['','','',''],
                error: false,
                focus: undefined
                    ...
                    <View style={Styles.otpView} >
                        {this.state.otp.map((item, index)=>{
                            ...
                            return(
                                <TextInput 
                                key={`otp${index}`} 
                                style={[Styles.otpField,{ borderColor: error && this.state.otp[index]==''? 'red' : focus==index?EStyleSheet.value('$theme1'): '#D0D0D0'} ]} 
                                maxLength={1} keyboardType='number-pad' value={item} placeholder='*' placeholderTextColor='#757575' underlineColorAndroid='transparent' ref={(ref) =>  this['otp'+index] = ref } 
                                onFocus={()=> {
                        ...
                        <TouchableOpacity onPress={()=>reSend()} >
                            <Text style={{fontSize:dp(14), color:EStyleSheet.value('$theme1'),  }} >Resend Otp</Text>
                        </TouchableOpacity>
